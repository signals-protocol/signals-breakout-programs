...
describe("Collateral Withdrawal", () => {
  ...
  it("관리자가 마켓 담보금을 출금할 수 있어야 합니다", async () => {
  ...
  it("컨트랙트에 담보 이벤트가 발생해야 합니다", async () => {
    // 먼저 토큰 구매하여 담보금 추가
    const binIndices = [0];
    const amounts = [new BN(100_000_000_000)];
    ...
    const collateralAmount = marketInfo.collateralBalance;

    // 이벤트를 확인할 방법이 없으므로 계정 상태 변경만 확인
    // 담보금 출금
    await env.program.methods
      .withdrawCollateral(new BN(env.marketId))
      .accounts({
